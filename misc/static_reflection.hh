...
#define DEFINE_STRUCT_SCHEMA(Struct, ...)        \
      template <>                                    \
      inline constexpr auto simple_ref::StructSchema<Struct>() { \
        using _Struct = Struct;                      \
        return std::make_tuple(__VA_ARGS__);         \
      }

#define DEFINE_STRUCT_FIELD_NAME(StructField, FieldName) \
...
#define DEFINE_STRUCT_FIELD(StructField) \
    ...
    template <typename T>
    constexpr bool has_schema = std::tuple_size<decltype(simple_ref::StructSchema<T>())>::value;
}
